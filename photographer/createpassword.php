...
if(isset($_POST['email'])){
  ...
  if ($mail->send()) {
      ...
      echo '<script type="text/javascript">alert("Email sent."); window.location = "otppassword.php";</script>';
      exit;
  } else {
      $status = "failed";
...
<body style="background:rgb(191,34,58););">
    <div class="bg-white shadow login-card" style="background: rgb(255,255,255);margin-top: 110px;"><img class="border rounded-circle border-white profile-img-card" src="assets/img/password.png" style="width: 150px;height: 149px;">
        <p class="profile-name-card" style="font-size: 23px;color: rgb(191,34,58);font-family: Alata, sans-serif;font-weight: 60;">Supply Username/Email Password?</p>
        <form class="form-signin"  action="" name="form" onsubmit="return validateForm();" method="post"><span class="reauth-email"> </span><input class="form-control" name="email" id="email" type="email" id="inputEmail"placeholder="Email address" autofocus="" style="border-top-color: rgb(191,34,58);border-right-color: rgb(191,34,58);border-bottom-color: rgb(191,34,58);border-left-color: rgb(191,34,58);font-family: Alata, sans-serif;"><span id="uerror"></span>
            <div class="checkbox" style="font-weight: 60;"></div><button class="btn btn-primary btn-block btn-lg btn-signin" type="submit" style="background: rgb(191,34,58);font-family: Alata, sans-serif;font-weight: 60;"> Create password</button>
        </form><a class="forgot-password" href="login.php" style="color: rgb(191,34,58);font-family: Alata, sans-serif;"><i class="far fa-arrow-alt-circle-left"></i>&nbsp;Back</a>
    </div>
